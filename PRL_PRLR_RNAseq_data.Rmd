...
hyp.prlr<-prl.long %>% filter(focal=="y" & tissue=="hypothalamus" & geneid=="prlr") %>%
  ...
  ggplot(aes(x = timept, y = avg_exp)) +
  geom_point(aes(x=timept, y=log.exp, color=sex, fill=sex), data = prl.long %>% filter(tissue=="hypothalamus" & geneid=="prlr" & focal=="y"),inherit.aes = FALSE,size =4, alpha  =0.75, pch=21)+ scale_fill_manual(values=c("mediumpurple2","sienna1")) + scale_color_manual(values=c("mediumpurple2","sienna4"))+ 
...
pit.prl<-prl.long %>% 
  ...
  summarise(avg_exp = mean(log.exp, na.rm = TRUE)) %>%
  ggplot(aes(x = timept, y = avg_exp)) +
    geom_point(aes(x=timept, y=log.exp, color=sex, fill=sex), data = prl.long %>% filter(tissue=="pituitary" & geneid=="prl" & focal=="y"),inherit.aes = FALSE,size =4, alpha  =0.75, pch=21)+ scale_fill_manual(values=c("mediumpurple2","sienna1")) + scale_color_manual(values=c("mediumpurple2","sienna4"))+ 
    geom_line(col="grey43", size=1, alpha=1) + 
    scale_x_continuous(breaks=c(2,3,4,6,7,9,11), labels=c("bldg", "lay", "inc.d3", "inc.d9", "m.inc.d8",  "inc.d17", "hatch"))+ 
  ylim(6,14)+
    theme(axis.text.x = element_text(angle = 90, hjust = 1)) + 
...
pit.prlr<-prl.long %>% 
  ...
  summarise(avg_exp = mean(log.exp, na.rm = TRUE)) %>%
  ggplot(aes(x = timept, y = avg_exp)) +
    geom_point(aes(x=timept, y=log.exp, color=sex, fill=sex), data = prl.long %>% filter(tissue=="pituitary" & geneid=="prlr" & focal=="y"),inherit.aes = FALSE,size =4, alpha  =0.75, pch=21)+ scale_fill_manual(values=c("mediumpurple2","sienna1")) + scale_color_manual(values=c("mediumpurple2","sienna4"))+ 
    geom_line(col="grey43", size=1, alpha=1) + 
    scale_x_continuous(breaks=c(2,3,4,6,7,9,11), labels=c("bldg", "lay", "inc.d3", "inc.d9", "m.inc.d8",  "inc.d17", "hatch"))+ 
    theme(axis.text.x = element_text(angle = 90, hjust = 1)) + 
    labs(title="Pit PRLR") +
...
#CROPS PRL
cp<-crops.long %>% 
  filter(gene=="PRL_ddCT")%>%
  group_by(timept) %>%
  summarise(avg_exp = round(mean(ddCT, na.rm = TRUE),2))
crp.prl<-cp %>%
  ggplot(aes(x = timept, y = avg_exp)) +
    geom_point(aes(x=timept, y=-(ddCT), color=Sex, fill=Sex), data = crops.long %>% filter(gene=="PRL_ddCT"),inherit.aes = FALSE,size =4, alpha  =0.75, pch=21)+ scale_fill_manual(values=c("mediumpurple2","sienna1")) + scale_color_manual(values=c("mediumpurple2","sienna4"))+ 
    geom_line(col="grey43", size=1, alpha=1) + 
    theme(axis.text.x = element_text(angle = 90, hjust = 1)) + 
    labs(title="Crop ePRL") +
    theme_minimal()
print(crp.prl)

#CROPS PRLR
cr<-crops.long %>% 
  filter(gene=="PRLR_ddCT")%>%
  group_by(timept) %>%
  summarise(avg_exp = round(mean(-(ddCT), na.rm = TRUE),2))
crp.prlr<-cr %>%
  ggplot(aes(x = timept, y = avg_exp)) +
    geom_point(aes(x=timept, y=-(ddCT), color=Sex, fill=Sex), data = crops.long %>% filter(gene=="PRLR_ddCT"),inherit.aes = FALSE,size =4, alpha  =0.75, pch=21)+ scale_fill_manual(values=c("mediumpurple2","sienna1")) + scale_color_manual(values=c("mediumpurple2","sienna4"))+ 
    geom_line(col="grey43", size=1, alpha=1) + 
    theme(axis.text.x = element_text(angle = 90, hjust = 1)) + 
    labs(title="Crop PRLR") +
    theme_minimal()
print(crp.prlr)
```
...
#only an effect of stage on Hyp PRL
summary(glm(log.exp~stage, data=hprl, family="gaussian")) 
...
pairs(lsmeans(m.hprlr, ~ stage)) #females less than males
...
ph_with_vg(code = print(all.genes), type="body", bg = "transparent") %>%
  add_slide(layout = "Title and Content", master = "Office Theme") %>% 
ph_with_vg(code = print(crp.prlr), type="body", bg = "transparent") %>%
