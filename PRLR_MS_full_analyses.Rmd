...
```{r hyp figures, echo=FALSE, message=FALSE, warning=FALSE}
#only run once
...
anova(m1.cp, test="Chisq") 

summary(m1.cp) #comparisons to female, building (default reference)

#Changing references 
cp$Stage <- relevel(cp$Stage, ref = "Hatch") #Hatch now reference
m2.cp <-glm(log.exp.neg~Stage*Sex, data=cp, family="gaussian")
summary(m2.cp)

cp$Stage <- relevel(cp$Stage, ref = "Inc_D3") #Inc_D3 now reference
m3.cp <-glm(log.exp.neg~Stage*Sex, data=cp, family="gaussian")
summary(m3.cp)

cp$Stage <- relevel(cp$Stage, ref = "Manip_D8") #Manip D8 now reference
m4.cp <-glm(log.exp.neg~Stage*Sex, data=cp, family="gaussian")
summary(m4.cp)
```
###PRLR Crop
"ANOVA" : weak trend towards stage
Not really anything going on in downstream comparisons. 
**Why then did we find something significant before?**  
In previous analyses for SfN, I ran my models using -ddCT as a dependent variable, rather than log transformed -ddCT.   
With log-transformed data, the effect disappeared.   
...
#Changing references 
cr$Stage <- relevel(cr$Stage, ref = "Hatch") #Hatch now reference
m2.cr <-glm(log.exp.neg~Stage*Sex, data=cr, family="gaussian")
summary(m2.cr)

cr$Stage <- relevel(cr$Stage, ref = "Inc_D3") #Inc_D3 now reference
m3.cr <-glm(log.exp.neg~Stage*Sex, data=cr, family="gaussian")
summary(m3.cr)

cr$Stage <- relevel(cr$Stage, ref = "Manip_D8") #Hatch now reference
m4.cr <-glm(log.exp.neg~Stage*Sex, data=cr, family="gaussian")
summary(m4.cr)
